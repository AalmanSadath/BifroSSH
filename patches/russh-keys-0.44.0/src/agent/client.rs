    ...
    pub async fn request_identities(&mut self) -> Result<Vec<PublicKey>, Error> {
        ...
        if self.buf[0] == msg::IDENTITIES_ANSWER {
            ...
            for _ in 0..n {
                ...
                // PATCH (BifroSSH): skip identities this crate cannot parse
                // instead of failing the whole listing.
                //
                // Upstream pushes `parse_public_key(..)?` here, so a single
                // unsupported key aborts request_identities entirely. Agents
                // routinely hold FIDO/security-key identities
                // (sk-ssh-ed25519@openssh.com, sk-ecdsa-*), which this crate
                // has no support for, and one of them would otherwise make
                // every other key in the agent unusable.
                //
                // The blob and comment are read before parsing, so the reader
                // stays in sync and the remaining identities still decode.
                match key::parse_public_key(key_blob, Some(SignatureHash::SHA2_512)) {
                    Ok(key) => keys.push(key),
                    Err(e) => {
                        debug!("ignoring unsupported agent identity: {:?}", e);
                    }
                }
            }
        }
