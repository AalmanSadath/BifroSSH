...
fn path_to_asset_key(path: &Path) -> AssetKey {
...
impl From<&str> for AssetKey {
  fn from(path: &str) -> Self {
    path_to_asset_key(Path::new(path))
  }
}
...
impl From<&std::path::Path> for AssetKey {
  fn from(path: &std::path::Path) -> Self {
    path_to_asset_key(path)
  }
}
...
impl From<&std::path::PathBuf> for AssetKey {
  fn from(path: &std::path::PathBuf) -> Self {
    path_to_asset_key(path.as_path())
  }
}

impl From<&std::ffi::OsStr> for AssetKey {
  fn from(path: &std::ffi::OsStr) -> Self {
    path_to_asset_key(Path::new(path))
  }
}

impl From<std::ffi::OsString> for AssetKey {
  fn from(path: std::ffi::OsString) -> Self {
    path_to_asset_key(Path::new(&path))
  }
}
